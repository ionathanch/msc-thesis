...
since for their underlying type theories, consistency coincides with the eventual termination of all functions\punctstack{.}%
\footnote{Some proof assistants such as Lean are consistent but may have nonterminating functions
due to other features of their type theories.}
...
Some proof assistants (Coq, for one) will inline functions and even unfold recursive functions
...
The one this paper discusses on is \emph{sized typing}\punctstack{,}%
...
By type preservation, consistency of the target type theory then implies that of the source.
...
There are many variations on dependent type theory with a variety of features;
...
\begin{itemize}
  ...
  \item Extensional Type Theory (ETT) has a noncumulative universe hierarchy,
    dependent pair types, a propositional equality type,
    and most importantly, features equality reflection.
\end{itemize}

The target language to which \lang is translated in the syntactic model is
the Extensional Calculus of Inductive Constructions (\CICE),
...
as representatives in place of inductive types
to more clearly see how concrete types are modelled
and to avoid the additional burden that comes with generalizing to inductive types.
...
\footnotetext[4]{\label{foot:CICE-consistency} Under the postulate that \CICE is consistent.}
...
Along with higher-rank sizes, we also include \emph{bounded} size quantification $\Funtype<{\alpha}{s}{\tau}$
...
Although we don't include an infinite size,
...
is given in \cref{subsec:infinity}.
On the other hand, the lack of an infinite size prevents
certain infinitary constructs from being expressed;
\cref{subsec:examples:limitations} gives a concrete example.

In the syntactic model, sizes and their order is translated to inductive definitions in \CICE,
and quantification over (bounded) sizes is simply translated
...
which deal with the universe levels of the inductive types and the lack of an infinite size,
as well directions for future investigation.

In these sections to follow, initial appearances in the text of new notation is highlighted in grey.
