...
Again, I assume that shadowing is disallowed,
so that the variables in the environments are unique.
...
or of a size variable by a size expression.
...
which extends congruence of reduction.
...
but the term being substituted in must be $e$.
\end{corollary}
...
with dotted lines overlaid with $\blacktriangleright$ representing the judgement,
the solid arrow representing reduction $\rhd$, and
...
\begin{proof}
See \citet[Theorem 3.10]{confluence}.
\end{proof}
...
The judgement I use to satisfy the Z property is an extension of
...
which is defined as applying the first possible rule from left to right, top to bottom.
This makes complete development decidable and deterministic (up to subsizing)
...
\end{itemize}
The remaining cases for congruence of reduction not covered here are similar to those above,
...
For concision, I prove all inversion principles for any arbitrary typing rule
...
as well as a few inversion principles for the closure of reduction.
...
\begin{corollary}[Inversion on closure of reduction (\rref{forall, forall<})] \label{lem:inversion-red*}
...
\begin{enumerate}[noitemsep, label=\textbf{Case}, leftmargin=*, labelindent=\parindent]
  ...
  \item[\textbf{Cases}] \rref*{acum-prop}, \rref*{acum-type}.
    Since $\Prop$ and $\Type{}$ don't reduce any further,
    trivial by \rref*{acum-prop} or \rref*{acum-type} respectively with \rref{red*-refl}.
  \item \rref*{acum-pi}. \vspace{-\baselineskip}
    ...
    \end{mathpar}
    By \cref{lem:inversion-red}, we have
  ...
  \item \rref*{acum-forall}, \rref*{acum-forall<}. Similar to the case for \rref*{acum-pi},
    using \cref{lem:inversion-red*} in place of \cref{lem:inversion-red}. \qedhere
\end{enumerate}
\end{proof}
...
of which there is only one case for each derivation.
\begin{mathpar}
\inferrule{
...
then there exist a type $\sigma$ and derivations $\mathcal{D}_i$ ending in $\mathcal{J}_i$
...
&\tau'''_2
\end{tikzcd}
\caption[Diagram of proof of \cref*{lem:replacement-reduction-3}]{Diagram of proof of \cref{lem:replacement-reduction-3}}
...
\begin{itemize}[noitemsep, label=\textbf{Case}, leftmargin=*, labelindent=\parindent]
  ...
  \item \rref*{red-cong}.
    ...
    and by \rref{let}, we have $\type{\Phi; \Gamma}{\letin{x}{\sigma}{e'_1}{e_2}}{\subst{\tau'}{x}{e'_1}}$.
    By \nameref{lem:congruence},
  ...
  \item[\textbf{Cases}] \textbf{remaining}.
    Similar to the case for $\beta$-reduction (application of bounded sizes). \qedhere
\end{itemize}
\end{proof}
...
and they're similar to the proofs of the analogous lemmas for \lang.
...
\end{itemize}
The above also apply when the environment contains $\defineT{\xT}{\sigmaT_2}{\eT}$
...
\end{itemize}
The above also apply when the environment contains $\defineT{\xT}{\sigmaT}{\eT}$
...
\begin{postulate}[Consistency of \CICE] \label{postulate:consistency-cice}
...
\begin{itemize}[noitemsep, label=\textbf{Case}, leftmargin=*, labelindent=\parindent]
  ...
  \item[\textbf{Cases}] \rref*{pi}, \rref*{lam}, \rref*{app}, \rref*{let}, \rref*{forall}, \rref*{slam}, \rref*{sapp}, \rref*{forall<}, \rref*{slam<}, \rref*{wft}, \rref*{case-nat}, \rref*{case-wft}.
    ...
    then substitution of the subterms in neither \lang nor in \CICE would occur,
...
we also need compositionality when substituting sizes.
...
The right-hand side only has a single substitution,
...
\begin{itemize}[noitemsep, label=\textbf{Case}, leftmargin=*, labelindent=\parindent]
  ...
  \item[\textbf{Cases}] \rref*{sapp}, \rref*{forall<}, \rref*{slam<}, \rref*{nat}, \rref*{wft}.
    ...
    && \textit{by \cref{sublem:compos-size}} \\
    ...
    \end{align*}
  \item[\textbf{Cases}] \rref*{sapp<}, \rref*{zero}, \rref*{succ}, \rref*{sup}.
    In these cases, there are additional terms generated from translating subsizing judgements;
...
namely that a translation $\compile{e}$ is well typed with type $\tauT$,
and that $\tauT$ isn't necessarily the translation $\compile{\tau}$ of the type of $e$.
...
\begin{itemize}[noitemsep, label=\textbf{Case}, leftmargin=*, labelindent=\parindent]
  ...
  \item \rref{acum-pi}.
    ...
    $\type{\compile{\Phi}, \compile{\Gamma}}{\funtypeT{\xT}{\compile{\sigma}}{\compile{\tau_i}}}{\compile{U_i}}$
    ...
    \begin{itemize}[noitemsep]
      ...
      \item $\type{\compile{\Phi}, \compile{\Gamma}, \annotT{\xT}{\compile{\sigma}}}{\compile{\tau_i}}{\UT''_i}$.
    ...
    $\subtype{\compile{\Phi}, \compile{\Gamma}, \annotT{\xT}{\compile{\sigma}}}{\compile{\tau_1}}{\compile{\tau_2}}$.
    By \rref{equiv-refl}, we have $\defeq{\compile{\Phi}, \compile{\Gamma}}{\compile{\sigma}}{\compile{\sigma}}{\UT'}$.
    Then by \rref{subtype-pi}, we have
    $\subtype{\compile{\Phi}, \compile{\Gamma}}{\funtypeT{\xT}{\compile{\sigma}}{\compile{\tau_1}}}{\funtypeT{\xT}{\compile{\sigma}}{\compile{\tau_2}}}$.
  \item \rref{acum-forall}.
    ...
    $\type{\compile{\Phi}, \compile{\Gamma}}{\funtypeT{\alphaT}{\SizeT}{\compile{\tau_i}}}{\UT_i}$
    for $i = 1, 2$, omitting unneeded judgements, we have
    \begin{itemize}[noitemsep]
      \item $\type{\Phi, \alpha; \Gamma}{\tau_i}{U''_i}$,
      \item $\type{\compile{\Phi}, \compile{\Gamma}}{\SizeT}{\UT'}$, and
      \item $\type{\compile{\Phi}, \compile{\Gamma}, \annotT{\alphaT}{\SizeT}}{\compile{\tau_i}}{\UT''_i}$.
    \end{itemize}
    By the induction hypothesis on the premise using the above, we have
    $\subtype{\compile{\Phi}, \compile{\Gamma}, \annotT{\alphaT}{\SizeT}}{\compile{\tau_1}}{\compile{\tau_2}}$.
    By \rref{equiv-refl}, we have $\defeq{\compile{\Phi}, \compile{\Gamma}}{\SizeT}{\SizeT}{\UT'}$.
    Then by \rref{subtype-pi}, we have
    $\subtype{\compile{\Phi}, \compile{\Gamma}}{\funtypeT{\alphaT}{\SizeT}{\compile{\tau_1}}}{\funtypeT{\alphaT}{\SizeT}{\compile{\tau_2}}}$.
  \item \rref{acum-forall<}.
    \vspace{-\baselineskip}
    \begin{mathpar}
    \inferrule{
    ...
    $\type{\compile{\Phi}, \compile{\Gamma}}{\funtypeT{\alphaT}{\SizeT}{\funtypeT{\alphaT^*}{\alphaT \szltT \compile{s}}{\compile{\tau_i}}}}{\UT_i}$
    for $i = 1, 2$, omitting unneeded judgements, we have
    \begin{itemize}[noitemsep]
      ...
      \item $\type{\compile{\Phi}, \compile{\Gamma}, \annotT{\alphaT}{\SizeT}, \annotT{\alphaT^*}{\alphaT \szltT \compile{s}}}{\compile{\tau_i}}{\UT'''_i}$.
    \end{itemize}
    By the induction hypothesis on the premise using the above, we have
    $\subtype{\compile{\Phi}, \compile{\Gamma}, \annotT{\alphaT}{\SizeT}, \annotT{\alphaT^*}{\alphaT \szltT \compile{s}}}{\compile{\tau_1}}{\compile{\tau_2}}$.
    By \rref{equiv-refl}, we have $\defeq{\compile{\Phi}, \compile{\Gamma}}{\SizeT}{\SizeT}{\UT'}$
    ...
    $$\subtype{\compile{\Phi}, \compile{\Gamma}}{\funtypeT{\alphaT}{\SizeT}{\funtypeT{\alphaT^*}{\alphaT \szltT \compile{s}}{\compile{\tau_1}}}}{\funtypeT{\alphaT}{\SizeT}{\funtypeT{\alphaT^*}{\alphaT \szltT \compile{s}}{\compile{\tau_2}}}}.$$
    \qedhere
\end{itemize}
\end{proof}
...
% TODO: clearpage
\clearpage
\begin{lemma}[Preservation of sizes] \label{lem:pres-size} \hfill
\begin{enumerate}[noitemsep]
...
\section{Consistency of \lang}

Now that type preservation has been proven,
we know that \lang is consistent with respect to the consistency of \CICE.
The proof of consistency is reproduced below.

\begin{theorem}[Consistency of \lang] \label{thm:overview:consistency} \hfill \\
There exists no term $e$ such that \mbox{$\type{\mt \mathbin{;} \mt}{e}{\funtype{P}{\Prop}{P}}$}.
\end{theorem}
\begin{proof}
Suppose that there were such a term $e$.
By \nameref{thm:pres-typing}, we would have that
$\type{\mt}{\compile{e}}{\funtypeT{\PT}{\PropT}{\PT}}$ holds.
However, this contradicts \nameref{postulate:consistency-cice},
