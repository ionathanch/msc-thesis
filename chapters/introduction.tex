...
The equality judgement, also known as definitional equality,
is \emph{not} the same as propositional equality:
the former is a relation between two terms that's checked by the type checker,
while the latter is a type that may be inhabited by a proof term.
...
Equality is sometimes described in terms of reduction judgements,
governed by rules of computation.
...
to normalization and thus type checking.
...
The type of such a function could be
...
which is a rule for definitional equality between terms requiring as premise
a proof of propositional equality between them.
