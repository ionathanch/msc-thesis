...
The latter is also known as \emph{definitional equality}\index{definitional equality},
...
A type theory can also have a notion of \emph{propositional equality}\index{propositional equality},
...
Two definitionally equal terms are propositionally equal by the \rref*{reflexivity} proof term
...
%
\begin{mathpar}
\inferrule[\rlabel*{reflexivity}]{
  \defeq{\Phi; \Gamma}{e_1}{e_2}{\tau}
}{
  \type{\Phi; \Gamma}{\refl{}}{\eq{e_1}{\tau}{e_2}}
}
\and
\inferrule[\rlabel*{reflection}]{
  \type{\Phi; \Gamma}{p}{\eq{e_1}{\tau}{e_2}}
}{
  \defeq{\Phi; \Gamma}{e_1}{e_2}{\tau}
}
\end{mathpar}
%
On the other hand, in an \emph{extensional} type theory, where we have the above equality \rref*{reflection} rule,
...
Finally, definitional/propositional \emph{proof irrelevance}\index{proof irrelevance}
...
Types that are propositionally proof irrelevant are also called \emph{mere propositions}\index{mere proposition}.
...
The alternative this thesis focuses on is \emph{sized typing}\punctstack{,}%
...
\begin{itemize}
  ...
  \item A \textbf{cumulative universe hierarchy}\index{cumulativity} such that $\Type{i}: \Type{i+1}$,
...
and a propositional equality\index{propositional equality} type.
...
For concision, I henceforth refer to \CICE's typed, declarative equality as \emph{equivalence}\index{equivalence},
and to \lang's untyped, algorithmic equality as \emph{conversion}\index{conversion}.

To prove consistency, this translation from \lang to \CICE must be \emph{type preserving}\index{type preservation}%
...
\begin{postulate}[Consistency of \CICE] \label{fact:consistency-cice} \hfill \\
There exists no term $\eT$ such that
\mbox{$\type{\mt}{\eT}{\funtypeT{\PT}{\PropT}{\PT}}$}.
\end{postulate}

\begin{theorem}[Consistency of \lang] \hfill \\
...
equality reflection\index{equality reflection} to \GCC using \rref{cum} in \CCE,
...
\textbf{\lang} & \checkmark* & \checkmark* & \checkmark* &
  \phantom{\textsuperscript{\labelcref{foot:CICE-consistency}}}%
  \checkmark*%
