\newcommand{\FigSyntaxInd}[1]{
  ...
  \end{align*}
  \caption{Syntax (\lang inductives)}
  \label{#1}
  \end{figure}
}
...
\newcommand{\FigRedInd}[1]{
  ...
  \centering
  \begin{mathpar}
  % This shrinks the space between the cdots and the first inference rule.
  % Don't ask me why it goes before the fbox instead of after. Don't touch it.
...
\newcommand{\FigTypingInd}[1]{
  ...
  \end{mathpar}
  \caption{Typing rules (\lang inductives)}
  \label{#1}
  \end{figure}
}