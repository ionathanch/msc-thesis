...
Fixpoint wfAcc P (IH : ∀ s, (∀ r, r < s → P r) → P s) s (acc : Acc s) : P s :=
  let p r rs := wfAcc P IH r (match acc return Acc r with | acc _ p => p r rs end)
  in IH s p.

Definition wfInd P IH s : P s := wfAcc P IH s (wf s).
\end{minted}
\end{singlespace}

\subsection{\texorpdfstring{$\WT$}{W} Types} \label{app:mechanization:coq:W}
...
Definition ac {A B} a (f : B a → { s & W A B s }) : { s & B a → W A B s} :=
  let f' b :=
    match projT2 (f b) with
    | sup _ _ _ r rs a f => sup _ _ _ r (transLeq rs (cocone b (reflLeq _))) a f
    end
  in existT _ (lim (λ b, projT1 (f b))) f'.
\end{minted}
\end{singlespace}
...
\subsection{\texorpdfstring{$\WT$}{W} Types} \label{app:mechanization:agda:W}
