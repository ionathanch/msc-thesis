...
\section{Inductive Types: Naturals and Well-Founded Trees}
...
when the sizes they are applied to are well formed;
their constructors are well typed when applied to smaller sizes;
and $\kw{case}$ expressions match on these size arguments in addition to the usual term arguments,
asserting within their branches that they are strictly smaller than the target's size.
As discussed, the bodies of fixpoints are only well typed
...
% TODO: mention why the type of naturals is in Type₁ and the type of wf trees are in axiom(U)